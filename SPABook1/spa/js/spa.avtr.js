...
spa.avtr = (function () { 
  ...
  onHeldmoveNav = function(event){
    var drap_map = stateMap.drag_map;
    if (!drap_map) { return false; }

    drap_map.top += event.px_delta_y;
    drap_map.left += event.px_delta_x;

    stateMap.$drag_target.css({top: drap_map.top, left: drap_map.left});
  };
  ...
  onLogout = function(){ jqueryMap.$container.empty(); };
  //--------------------------END EVENT HANDLERS -----------------
